...
public class BreakoutEngine extends SurfaceView implements  Runnable {
    ...
    public BreakoutEngine(Context context, int x, int y) {
        // This calls tthe default constructor to setup the rest of the object.
        super(context);
        ...
        // Initialize screenX and screenY becuase x and y are local
        screenX = x;
        screenY = y;
    ...
    // Runs when th OS calls onPause on BreakoutActivity method
    public void pause() {
        playing = false;
        try {
                gameThread.join();
      ...
      for (int i = 0; i < numBricks; i++) {
          if (bricks[i].getVisibility()) {
              ...
              if (RectF.intersects(bricks[i].getRect(), ball.getRect())) {
                  bricks[i].setInvisble();
                  ball.reverseYVelocity();
                  score = score + 10;
                  soundPool.play(explodeID,
                          1, 1, 0, 0, 1);
              }
          }
      }
